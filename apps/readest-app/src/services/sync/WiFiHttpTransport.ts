...
export class WiFiHttpTransport implements SyncTransport {
   ...
   * by `schema_version === 1`. Returns empty array on any error.
   */
   ...
   * Failures are silently swallowed — the next sync cycle will retry.
   */
  async push(kind: SyncCategory, rows: ReplicaRow[]): Promise<void> {
    ...
    try {
      ...
      // Best-effort push — next sync cycle will retry.
    }
  }
