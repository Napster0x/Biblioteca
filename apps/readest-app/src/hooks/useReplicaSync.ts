 ...
 *
 * Auto-syncs annotation, quote, and dictionary-entry replicas via WebDAV.
 ...
 * Does nothing when WebDAV is disabled or unconfigured.
 */
...
import {
  ...
  pushDictionaryImage,
  pullDictionaryImage,
} from '@/services/sync/replicaTransport';
...
export function useReplicaSync(): UseReplicaSyncResult {
   ...
   */
  const syncKind = useCallback(async (kind: SyncCategory): Promise<void> => {
    ...
    try {
      remoteRows = await pullReplicas(toClientConfig, rootPath, kind, since);
    } catch (e) {
        ...
        for (const row of remoteRows) {
          ...
          try {
            ...
            const localExists = await appService
              ...
              .catch(() => false);
            if (!localExists) {
              const imageBytes = await pullDictionaryImage(toClientConfig, rootPath, entryId);
              if (imageBytes) {
    ...
    if (outbox.length > 0) {
      try {
        await pushReplicas(toClientConfig, rootPath, kind, outbox);
        ...
        if (kind === 'dictionary-entry') {
          ...
          for (const row of outbox) {
            ...
            try {
              ...
              await pushDictionaryImage(toClientConfig, rootPath, entryId, bytes as ArrayBuffer);
            } catch {
