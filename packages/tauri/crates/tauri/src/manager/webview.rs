...
fn is_local_network_url(url: &url::Url) -> bool {
  match url.host() {
    ...
    // Skip IP-based URLs so physical devices can use adb reverse tunnel
    // to reach 127.0.0.1:3000 via USB without the Tauri proxy interfering.
    Some(url::Host::Ipv4(_)) | Some(url::Host::Ipv6(_)) => false,
    None => false,
  }
}
